...
class PhysicsSim():
    ...
    def reset(self):
        self.time = 0.0
        # Bug reported in Slack #project-5_quadcopter channel: should be using np.copy
    ...
    def get_propeler_thrust(self, rotor_speeds):
        ...
        for prop_number in range(4):
            ...
            #Bug reported by Dmytro Bagaiev on Slack. According to the paper, this J = V / n * D should be J = V / (n * D)
            #J = V / n * D
            J = V / (n * D)
    ...
    def next_timestep(self, rotor_speeds):
		...
		#Bug reported by Dmytro Bagaiev on Slack
