...
class DDPG():
    ...
    def step(self, action, reward, next_state, done):
        ...
        self.count += 1
        self.log.write('DDPG.step len(self.memory)=' + str(len(self.memory)) + ' total_reward=' + str(total_reward) + ' count=' + str(count))

        # Learn, if enough samples are available in memory
        if len(self.memory) > self.batch_size:
            experiences = self.memory.sample()
            self.log.write('DDPG.step len(self.memory) > self.batch_size' + str(len(self.memory)) + '>' + str(self.batch_size))
            print(experiences)
